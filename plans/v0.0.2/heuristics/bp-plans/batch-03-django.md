...
- [ ] **Detection approach:** line walk for `(?i)\bSECRET_KEY\s*=\s*`; require RHS string/bytes literal; skip `os.environ` / `getenv` / `config(` / `env(` style RHS; skip placeholder secrets if helper exists (`looksLikePlaceholderSecret`); **gate** with `looksDjangoSettings(unit)` or django content markers so Flask `BP-PY-17` remains primary for Flask-ish modules
- [ ] Path: `internal/lang/python/detectors/bad_practices/rules_django.go` — `detectBPPY22`
- [ ] Register: `RegisterRule("BP-PY-22", detectBPPY22)` in `init()`
- [ ] Unit hit: path `settings.py`, source with `SECRET_KEY = 'django-insecure-abc123-not-for-prod'` (or similar non-placeholder) → fires
- [ ] Unit miss: `SECRET_KEY = os.environ['SECRET_KEY']` → no fire
- [ ] Unit miss: non-settings / Flask-only path should not steal Flask cases (coordinate with existing `BP-PY-17` policy)
- [ ] Optional fixture: `tests/fixtures/python/bp/BP-PY-22-vulnerable.txt` / `-safe.txt`
- [ ] Metadata resolves via `MetadataForID("BP-PY-22")`; severity **high**
...
- [ ] **Detection approach (source heuristic):**
...
- [ ] Path: `rules_django.go` — `detectBPPY24`
- [ ] Register `BP-PY-24`
- [ ] Unit hit: `User.objects.raw(f"SELECT * FROM auth_user WHERE id = {uid}")`
- [ ] Unit hit: `cursor.execute("SELECT * FROM t WHERE id = %s" % (uid,))`
- [ ] Unit miss: `User.objects.raw("SELECT * FROM auth_user WHERE id = %s", [uid])`
- [ ] Optional fixture
- [ ] Severity **high**
...
- [ ] Path: `rules_django.go` — `detectBPPY25`
- [ ] Register `BP-PY-25`
- [ ] Unit hit: `mark_safe(user_input)` / `mark_safe(request.GET['q'])`
- [ ] Unit miss: `mark_safe("<br>")`
- [ ] Optional fixture
- [ ] Severity **high**
...
- [ ] Path: `rules_django.go` — `detectBPPY26`
- [ ] Register `BP-PY-26`
- [ ] Unit hit: `@csrf_exempt` + `def pay(request):` using `request.POST`
- [ ] Unit miss: view without `csrf_exempt`
- [ ] Optional fixture
- [ ] Severity **high**

### 2.4 Phase 2 validation

- [ ] All three rules registered + hit/miss tests green in isolation
- [ ] No double-registration / init order issues
...
- [ ] Path: `rules_django.go` — `detectBPPY27`
- [ ] Register `BP-PY-27`
- [ ] Unit hit: `User(**request.POST)` or `User.objects.create(**request.data)`
- [ ] Unit miss: `User.objects.create(username=request.POST['u'])`
- [ ] Optional fixture
- [ ] Severity **medium**
...
- [ ] **Detection approach (review-only, documented confidence):**
...
- [ ] Path: `rules_django.go` — `detectBPPY28`
- [ ] Register `BP-PY-28`
- [ ] Unit hit: simple for-loop over `Model.objects.all()` accessing `item.author.name` without select_related
- [ ] Unit miss: same with `.select_related('author')` on the queryset
- [ ] Optional fixture
- [ ] Severity **medium**; message should say heuristic/review-only

### 3.3 Phase 3 validation

- [ ] `27` and `28` registered; hit/miss tests
- [ ] Comments in code note N+1 is best-effort
...
- [ ] `TestBPRulesRegistered` (or django-specific test) includes `BP-PY-22`…`BP-PY-28` (and still `BP-PY-21`)
- [ ] Each id has non-nil `MetadataForID` / `MetadataFor`; pack `PackBadPractice`
...
- [ ] If `rules_django.go` or `scan_test.go` approaches limit: split tests to `rules_django_test.go`; move shared helpers only if needed
- [ ] Do **not** dump Django rules into `rules_framework.go` if that would bloat it; new file is the plan default
- [ ] Record line counts beside this row when closing the PR
...
- [ ] Optional smoke: scan with `languages = ["python"]` on a tiny settings snippet

### 4.4 PR hygiene

- [ ] One PR for this batch (`batch-03-django`)
- [ ] PR body under `plans/PR/v0.0.2/` (or link from parent README)
- [ ] `Relates to #53` / `#51`; do not claim full catalogue completion
- [ ] Update parent inventory / `python-heuristics-bp.md` rule table: `22`–`28` → `[x]` when proven
- [ ] Update `_inventory.json` implemented/missing lists when batch lands (if that file is maintained as live inventory)
