...
- [ ] Record baseline: `wc -l internal/lang/python/detectors/bad_practices/*.go`
- [ ] Choose targets:
  - [ ] `BP-PY-3`, `BP-PY-5` → `rules_core.go` (or document why not)
  - [ ] `BP-PY-14`, `BP-PY-15` → **new** `rules_prod.go` **or** `rules_core.go` with budget note
- [ ] If creating `rules_prod.go`: `package badpractices`, `init()` with `RegisterRule` only for 14/15

### 1.2 Shared helpers (only if needed)

- [ ] Reuse existing `codeLinesFacts`, `pushAt`, `isPythonTestFile`, call-arg helpers from `common.go` where present
- [ ] If adding keyword-arg scanners (timeout presence, context-manager assignment), keep helpers under 1500-line policy in `common.go` or local to rules file
...
- [ ] `RegisterRule("BP-PY-3", detectBPPY3)` in `rules_core.go` `init()` (alongside 1/2/4/6/7)
- [ ] Confirm `MetadataForID("BP-PY-3")` non-nil (catalogue already loaded)

### 2.2 Detect heuristic

Cite **detection_notes:** match `raise Exception(...)` / `raise BaseException(...)` outside tests; prefer specific types.

- [ ] Implement `detectBPPY3`:
...
- [ ] Message: catalogue-aligned (generic Exception loses type information)

### 2.3 Hit / miss tests

- [ ] Hit: `raise Exception("bad")` / `raise Exception` in non-test module → finding `BP-PY-3`
- [ ] Hit: `raise BaseException("x")` → finding
- [ ] Miss: `raise ValueError("bad")` → no `BP-PY-3`
- [ ] Miss: test file path (`test_*.py` / `*_test.py` / under `tests/`) even with generic raise → no finding (if skip policy applied)
- [ ] Path: extend `scan_test.go` (or split test file if size gate trips)
- [ ] Update `TestBPRulesRegistered` want slice to include `BP-PY-3`

### 2.4 Proof

- [ ] `go test ./internal/lang/python/detectors/bad_practices/ -count=1` covers BP-PY-3 cases
...
- [ ] `RegisterRule("BP-PY-5", detectBPPY5)` in `rules_core.go` `init()`
...
- [ ] Skip test files only if product policy matches other low-noise rules (document choice)

### 3.3 Hit / miss tests

- [ ] Hit: `from os.path import *` in `app.py` → `BP-PY-5`
- [ ] Miss: `from os.path import join, exists` → no finding
- [ ] Miss or hit per policy: `from .models import *` in `__init__.py` — assert chosen policy
- [ ] Register id in want-list

### 3.4 Proof

- [ ] Unit test green for hit/miss
...
- [ ] `RegisterRule("BP-PY-14", detectBPPY14)` in **`rules_prod.go`** (preferred) or chosen target `init()`
...
- [ ] Severity medium from metadata

### 4.3 Hit / miss tests

- [ ] Hit: `requests.get("https://example.com")` → `BP-PY-14`
- [ ] Hit: `requests.post(url, json=body)` without timeout → finding
- [ ] Miss: `requests.get(url, timeout=5)` → no finding
- [ ] Miss: `requests.get(url, timeout=(1, 3))` → no finding
- [ ] Optional hit: `session.get(url)` without timeout if detector covers Session
- [ ] Register id in want-list

### 4.4 Proof

- [ ] Unit test green; severity sample optional
...
- [ ] `RegisterRule("BP-PY-15", detectBPPY15)` same file as BP-PY-14 (`rules_prod.go` preferred)
...
- [ ] Implement `detectBPPY15` (v0 heuristic, document limits):
  - Needle: `AsyncClient`
...
- [ ] Miss:  
  ...
  ```
- [ ] Optional miss: assign + `await client.aclose()` if that path is implemented
- [ ] Register id in want-list
...
- [ ] `TestBPRulesRegistered` includes `BP-PY-3`, `BP-PY-5`, `BP-PY-14`, `BP-PY-15`
- [ ] Collision guard still rejects bare `BP-*`
- [ ] `MetadataFor` pack = `PackBadPractice` for each new id
- [ ] No edits required to `all.go` (still single `NewPythonBadPracticeScan()`)

### 6.2 File size check after batch

- [ ] `wc -l internal/lang/python/detectors/bad_practices/*.go`
- [ ] Assert no file > **2000** lines (hard max)
- [ ] Assert no file > **1500** lines (target); if any exceeds target, **split before merge** (e.g. move 14/15 to `rules_prod.go`, or split tests)
...
- [ ] `make test` — unchecked until green  
  **Evidence:** _(command + date + branch)_
- [ ] Optional: `go test ./internal/lang/python/... -count=1`
- [ ] PR: `Relates to #53` / `Relates to #51` (not `Closes #53` until remaining batches done or explicit partial criteria met)
- [ ] Update parent [README.md](./README.md) rollup Batch 01 checkboxes to `[x]` only after gates green
- [ ] Refresh `_inventory.json` implemented list if process uses it
