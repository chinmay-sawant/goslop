...
- [ ] Ensure target `rules_auth.go` exists (create if needed); keep ≤1500 lines projected
...
- [ ] Add `MetaCWE306` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-306", detectCWE306, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-306`
...
- [ ] Implement `detectCWE306` with needle prefilter
...
- [ ] **Unit hit:** vulnerable snippet → finding `CWE-306`
- [ ] Unit miss: safe pattern → no `CWE-306`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-306-vulnerable.txt` + `CWE-306-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
...
- [ ] Add `MetaCWE307` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-307", detectCWE307, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-307`
...
- [ ] Implement `detectCWE307` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-307`
- [ ] Unit miss: safe pattern → no `CWE-307`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-307-vulnerable.txt` + `CWE-307-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE346` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-346", detectCWE346, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-346`
...
- [ ] Implement `detectCWE346` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-346`
- [ ] Unit miss: safe pattern → no `CWE-346`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-346-vulnerable.txt` + `CWE-346-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE359` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-359", detectCWE359, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-359`
...
- [ ] Implement `detectCWE359` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-359`
- [ ] Unit miss: safe pattern → no `CWE-359`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-359-vulnerable.txt` + `CWE-359-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE613` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-613", detectCWE613, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-613`
...
- [ ] Implement `detectCWE613` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-613`
- [ ] Unit miss: safe pattern → no `CWE-613`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-613-vulnerable.txt` + `CWE-613-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE565` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-565", detectCWE565, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-565`
...
- [ ] Implement `detectCWE565` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-565`
- [ ] Unit miss: safe pattern → no `CWE-565`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-565-vulnerable.txt` + `CWE-565-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE807` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-807", detectCWE807, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-807`
...
- [ ] Implement `detectCWE807` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-807`
- [ ] Unit miss: safe pattern → no `CWE-807`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-807-vulnerable.txt` + `CWE-807-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE698` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-698", detectCWE698, &Meta…, gates...)` in `rules_auth.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-698`
...
- [ ] Implement `detectCWE698` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-698`
- [ ] Unit miss: safe pattern → no `CWE-698`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-698-vulnerable.txt` + `CWE-698-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
