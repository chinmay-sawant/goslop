...
- [ ] Ensure target `rules_crypto.go` exists (create if needed); keep ≤1500 lines projected
...
- [ ] Add `MetaCWE295` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-295", detectCWE295, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-295`
...
- [ ] Implement `detectCWE295` with needle prefilter
...
- [ ] **Unit hit:** vulnerable snippet → finding `CWE-295`
- [ ] Unit miss: safe pattern → no `CWE-295`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-295-vulnerable.txt` + `CWE-295-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
...
- [ ] Add `MetaCWE328` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-328", detectCWE328, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-328`
...
- [ ] Implement `detectCWE328` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-328`
- [ ] Unit miss: safe pattern → no `CWE-328`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-328-vulnerable.txt` + `CWE-328-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE335` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-335", detectCWE335, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-335`

### Detect heuristic

- Cite **detection_notes** (abbrev): MITRE detection methods: Automated Static Analysis. Automated static analysis, commonly referred to as Static Application Security Testing (SAST), can find some instances of this weakness by analyzing source code (or bin…
- Suggested sinks/patterns: `random.seed(0)`, `random.seed(constant)`, `numpy.random.seed(fixed) for security tokens`
- [ ] Implement `detectCWE335` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-335`
- [ ] Unit miss: safe pattern → no `CWE-335`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-335-vulnerable.txt` + `CWE-335-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE338` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-338", detectCWE338, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-338`
...
- [ ] Implement `detectCWE338` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-338`
- [ ] Unit miss: safe pattern → no `CWE-338`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-338-vulnerable.txt` + `CWE-338-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE347` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-347", detectCWE347, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-347`
...
- [ ] Implement `detectCWE347` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-347`
- [ ] Unit miss: safe pattern → no `CWE-347`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-347-vulnerable.txt` + `CWE-347-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE1204` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-1204", detectCWE1204, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-1204`

### Detect heuristic

- Cite **detection_notes** (abbrev): MITRE detection methods: Automated Static Analysis. Automated static analysis, commonly referred to as Static Application Security Testing (SAST), can find some instances of this weakness by analyzing source code (or bin…
- Suggested sinks/patterns: `AES.new(..., MODE_CBC, iv=b'0'*16)`, `fixed IV literal`, `Crypto.Cipher with constant iv`
- [ ] Implement `detectCWE1204` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-1204`
- [ ] Unit miss: safe pattern → no `CWE-1204`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-1204-vulnerable.txt` + `CWE-1204-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE1240` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-1240", detectCWE1240, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-1240`
...
- [ ] Implement `detectCWE1240` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-1240`
- [ ] Unit miss: safe pattern → no `CWE-1240`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-1240-vulnerable.txt` + `CWE-1240-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE1241` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-1241", detectCWE1241, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-1241`
...
- [ ] Implement `detectCWE1241` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-1241`
- [ ] Unit miss: safe pattern → no `CWE-1241`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-1241-vulnerable.txt` + `CWE-1241-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE1392` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-1392", detectCWE1392, &Meta…, gates...)` in `rules_crypto.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-1392`
...
- [ ] Implement `detectCWE1392` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-1392`
- [ ] Unit miss: safe pattern → no `CWE-1392`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-1392-vulnerable.txt` + `CWE-1392-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
