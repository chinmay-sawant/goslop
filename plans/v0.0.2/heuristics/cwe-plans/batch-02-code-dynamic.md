...
- [ ] Ensure target `rules_code_dynamic.go` exists (create if needed); keep ≤1500 lines projected
...
- [ ] Add `MetaCWE749` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-749", detectCWE749, &Meta…, gates...)` in `rules_code_dynamic.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-749`
...
- [ ] Implement `detectCWE749` with needle prefilter
...
- [ ] **Unit hit:** vulnerable snippet → finding `CWE-749`
- [ ] Unit miss: safe pattern → no `CWE-749`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-749-vulnerable.txt` + `CWE-749-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
...
- [ ] Add `MetaCWE829` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-829", detectCWE829, &Meta…, gates...)` in `rules_code_dynamic.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-829`
...
- [ ] Implement `detectCWE829` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-829`
- [ ] Unit miss: safe pattern → no `CWE-829`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-829-vulnerable.txt` + `CWE-829-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE695` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-695", detectCWE695, &Meta…, gates...)` in `rules_code_dynamic.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-695`

### Detect heuristic

- Cite **detection_notes** (abbrev): MITRE detection methods: Automated Static Analysis. Automated static analysis, commonly referred to as Static Application Security Testing (SAST), can find some instances of this weakness by analyzing source code (or bin…
- Suggested sinks/patterns: `ctypes.`, `cffi`, `mmap.`, `os.system already CWE-78`
- [ ] Implement `detectCWE695` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-695`
- [ ] Unit miss: safe pattern → no `CWE-695`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-695-vulnerable.txt` + `CWE-695-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE214` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-214", detectCWE214, &Meta…, gates...)` in `rules_code_dynamic.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-214`
...
- [ ] Implement `detectCWE214` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-214`
- [ ] Unit miss: safe pattern → no `CWE-214`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-214-vulnerable.txt` + `CWE-214-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
...
- [ ] Add `MetaCWE215` in `metadata.go` (or domain metadata file) — title/name match catalogue
- [ ] `RegisterRule("CWE-215", detectCWE215, &Meta…, gates...)` in `rules_code_dynamic.go` `init()`
- [ ] Confirm ID exists in chunk JSON key `CWE-215`

### Detect heuristic

- Cite **detection_notes** (abbrev): MITRE detection methods: Automated Static Analysis. Automated static analysis, commonly referred to as Static Application Security Testing (SAST), can find some instances of this weakness by analyzing source code (or bin…
- Suggested sinks/patterns: `print(password`, `logging.debug(...secret`, `pdb`, `print(SECRET_KEY`, `logger.debug api_key`
- [ ] Implement `detectCWE215` with needle prefilter
- [ ] Prefer high-signal positive; document safe suppressions
- [ ] Message catalogue-aligned; confidence documented

### Hit / miss tests + fixtures (triad)

- [ ] **Unit hit:** vulnerable snippet → finding `CWE-215`
- [ ] Unit miss: safe pattern → no `CWE-215`
- [ ] **Fixtures:** `tests/fixtures/python/cwe/CWE-215-vulnerable.txt` + `CWE-215-safe.txt` (`lang: python` header; same `.txt` format as `tests/fixtures/python/bp/`)
- [ ] **Integration:** pair auto-discovered by `DiscoverPythonCWECases` → `TestPythonCWEFixturesMatrix` in `tests/integration/python/cwe_matrix_test.go`
- [ ] Run: `go test ./internal/lang/python/detectors/cwe/ -count=1` and `go test ./tests/integration/python/ -count=1` (or `make integration-python`)
