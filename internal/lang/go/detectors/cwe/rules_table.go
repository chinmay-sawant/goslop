...
var cweNeedleRules = []cweNeedleRule{
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"c.Header(\"Location\"", "c.Query(\"next\")"},
			{"c.Header(\"Location\"", "c.Query(\"return\")"},
		},
	...
	},
	{
		...
		forbid: []string{".MatchString(", "invoice.Amount <= 0"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"role = strings.ToLower(role)", "stripNonAlnum.ReplaceAllString"},
			{"role = strings.ToLower(role)", "ReplaceAllString(role, \"\")"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"APIKey", "c.JSON(http.StatusOK, record)"},
			{"type userRecord struct", "c.JSON(http.StatusOK, record)"},
		},
		forbid: []string{"publicUser{", "publicRecord{"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"Salary", "c.JSON(http.StatusOK, profile)"},
		},
		forbid: []string{"guestProfile{", "directoryEntry{"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"target := c.Query(\"path\")", "_ = os.Remove(target)"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"password_hash FROM users", "hasExport"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"_ = os.Remove(path)", "c.JSON(200, gin.H{\"removed\": path})"},
		},
		forbid: []string{"stat.Uid", "os.Stat(path)"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"c.PostForm(\"auth_token\")", "c.JSON(200, gin.H{\"ok\": true})"},
			{"c.PostForm(\"auth_token\")", "c.Set(\"user\", \"alice\")"},
		},
		forbid: []string{"spentNonces"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"c.JSON(401, gin.H{\"error\": \"bad creds\"})", "c.PostForm(\"email\")"},
		},
		forbid: []string{"loginAttempts", "rate limited"},
	...
	},
	{
		...
		groups: []needleGroup{
			{"http.Error(w, \"method not allowed\", http.StatusMethodNotAllo"},
			{"if c.PostForm(\"password\") == \"\" {", "INSERT INTO wires"},
	...
	},
	{
		...
		forbid: []string{"allowedOrigins", "trustedOrigins"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"DELETE FROM tenants WHERE slug = ?", "func PurgeTenant("},
			{"{\"purged\":true}", "func PurgeTenant("},
		},
		forbid: []string{"X-Confirm-Delete", "X-Expected-Token", "X-Confirm-Purge"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"DecodeString(parts[1])", "json.Unmarshal(payload, &claims)"},
		},
		forbid: []string{"unsupported jwt algorithm", "header.Alg"},
	...
	},
	{
		...
		forbid: []string{"authentication required", "errors.New(\"authentication required\")"},
	...
	},
	{
		...
		forbid: []string{"requireAdmin", "X-Role"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"cmd := exec.Command(path)", "/tmp/worker.bin"},
			{"exec.Command(path)", "bundle_url"},
		},
		forbid: []string{"workerBundleSHA", "integrity check failed"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"\"password\": pass,", "c.PostForm(\"email\")"},
			{"\"email\":    c.PostForm(\"email\"),", "\"password\": pass,"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"role, _ := c.Cookie(\"role\")", "if role != \"admin\" {"},
		},
		forbid: []string{"roleCookie", "c.GetString(\"role\")"},
	...
	},
	{
		...
		forbid: []string{"revokedSessions[sid]", "ExpiresAt"},
	...
	},
	{
		...
		forbid: []string{"AND user_id = $2", "user_id = $2"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"var sharedDB *gorm.DB", "AdminPurge"},
			{"sharedDB.Exec(\"DELETE FROM audit_logs\")"},
			{"var sharedAuditStore", "func AdminPurge("},
		},
		forbid: []string{"readOnlyDB", "adminDB", "readOnlyAuditStore", "adminAuditStore"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"_, _ = db.Query(\"SELECT pg_sleep(1)\")", "db.Close()"},
			{"SELECT pg_sleep(1)", "_ = db.Close()"},
		},
		span: "db.Close()",
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"plugin.Open(path)", "c.Query(\"module_path\")"},
			{"path := c.Query(\"module_path\")", "plugin.Open(path)"},
		},
		forbid: []string{"allowedModules", "moduleRoot"},
	...
	},
	{
		...
		forbid: []string{"if !accountMFAPassed[email]", "if !acct.MFAPassed"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"appDB.Find(&widgets)"},
			{"var appDB *gorm.DB", "appDB.Find"},
		},
		forbid: []string{"if widgetDB == nil", "appDB == nil"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"c.BindJSON(&evt)", "AcceptWebhook"},
			{"if err := c.BindJSON(&evt); err != nil {"},
		},
		forbid: []string{"X-Signature", "hmac.New"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"userID := c.Query(\"user_id\")", "oauth_tokens"},
			{"c.Query(\"user_id\")", "INSERT INTO oauth_tokens"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"email := c.Query(\"email\")", "smtp.SendMail"},
			{"smtp.SendMail", "c.Query(\"token\")"},
		},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"var ledgerMu sync.Mutex", "func UpdateBalance("},
			{"ledgerMu.Lock()", "func UpdateBalance("},
		},
		forbid: []string{"ledgerMuSafe", "defer ledgerMuSafe.Unlock()"},
	...
	},
	{
		...
		groups: []needleGroup{
			...
			{"json.Unmarshal(body, &cfg)", "hook_configs"},
		},
		forbid: []string{"DisallowUnknownFields", "ParseRequestURI"},
